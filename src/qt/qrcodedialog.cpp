...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BOB")
    updateDisplayUnit();
}
...
{
    QString ret = QString("dobbscoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BOB unit input in lnReqAmount, we generate the URI with BOB as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BOB, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
