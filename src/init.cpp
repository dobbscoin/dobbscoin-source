...
void Shutdown()
{
    ...
    if (pwalletMain)
        bitdb.Flush(false);
    GenerateDobbscoins(false, NULL, 0);
#endif
    StopNode();
...
std::string HelpMessage(HelpMessageMode mode)
{
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "dobbscoind.pid") + "\n";
#endif
    ...
    strUsage += "  -port=<port>           " + strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 19985, 39985) + "\n";
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += "  -mintxfee=<amt>        " + strprintf(_("Fees (in BOB/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())) + "\n";
    strUsage += "  -paytxfee=<amt>        " + strprintf(_("Fee (in BOB/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "  -minerscrypt=<impl>    " + _("Scrypt implementation for coin generation: auto, generic, sse2, avx or avx2 (default: auto, the fastest this CPU supports)") + "\n";
#endif
    ...
    strUsage += "  -minrelaytxfee=<amt>   " + strprintf(_("Fees (in BOB/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 19984, 39984) + "\n";
    ...
    strUsage += "\n" + _("RPC SSL options: (see the Dobbscoin Wiki for SSL setup instructions)") + "\n";
...
{
    RenameThread("dobbscoin-loadblk");
...
/** Sanity checks
 *  Ensure that Dobbscoin is running in a usable environment with all
...
bool InitSanityCheck(void)
{
    if(!ECC_InitSanityCheck()) {
        InitError("OpenSSL appears to lack support for elliptic curve cryptography. For more "
                  "information, visit https://en.dobbscoin.it/wiki/OpenSSL_and_EC_Libraries");
        return false;
    }
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Dobbscoin Core is shutting down."));
...
#endif
    // Make sure only a single Dobbscoin process is using the data directory.
    boost::filesystem::path pathLockFile = GetDataDir() / ".lock";
    FILE* file = fopen(pathLockFile.string().c_str(), "a"); // empty lock file; created if it doesn't exist.
    if (file) fclose(file);
    static boost::interprocess::file_lock lock(pathLockFile.string().c_str());
    if (!lock.try_lock())
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. Dobbscoin Core is probably already running."), strDataDir));
#ifndef WIN32
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of Dobbscoin Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart Dobbscoin Core to complete") << "\n";
...
#ifdef ENABLE_WALLET
    // The miner's scrypt implementation, chosen and logged once. Block
    // validation does not use it.
    MinerScryptSelect(GetArg("-minerscrypt", "auto"));

    // Generate coins in the background
    if (pwalletMain)
        GenerateDobbscoins(GetBoolArg("-gen", false), pwalletMain, GetArg("-genproclimit", 1));
#endif
