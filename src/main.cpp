...
bool CTxOut::IsDust() const
{
    ...
    // so dust is a txout less than 54 uBOB
...
{
    //dobbshield. Designed to rid us of the sins of the NORMALS and their short term hashrate pumps
    unsigned int nProofOfWorkLimit = bnProofOfWorkLimit.GetCompact();
    int nHeight = pindexLast->nHeight + 1;
    
    int64 retargetTimespan = nTargetTimespan_v3;
    int64 retargetSpacing = nTargetSpacing_v3;
    int64 retargetInterval = nInterval_v3;
...
unsigned char pchMessageStart[4] = { 0xa0, 0xfb, 0x17, 0x83 };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("DobbscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("DobbscoinMiner : ProcessBlock, block not accepted");
    }
...
void static BitcoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running DobbscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("DobbscoinMiner terminated\n");
        throw;
    }
}
