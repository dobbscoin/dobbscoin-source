...
# Add python-dobbscoinrpc to module search path:
import os
import sys
sys.path.append(os.path.join(os.path.dirname(os.path.abspath(__file__)), "python-dobbscoinrpc"))
...
dobbscoind_processes = {}

def initialize_datadir(dirname, n):
    datadir = os.path.join(dirname, "node"+str(n))
    if not os.path.isdir(datadir):
        os.makedirs(datadir)
    with open(os.path.join(datadir, "dobbscoin.conf"), 'w') as f:
        f.write("regtest=1\n");
        f.write("rpcuser=rt\n");
...
def start_nodes(num_nodes, dirname, extra_args=None, rpchost=None):
    """
    Start multiple dobbscoinds, return RPC connections to them
    """
...
def stop_node(node, i):
    node.stop()
    dobbscoind_processes[i].wait()
    del dobbscoind_processes[i]
...
def wait_dobbscoinds():
    # Wait for all dobbscoinds to cleanly exit
    for dobbscoind in dobbscoind_processes.values():
        dobbscoind.wait()
    dobbscoind_processes.clear()
